...
public static class O5Decoder
{
    public const string RecordSpec = "O5";
    public const int RecordLength = 12293;
    private const int EntryCount = 816;
    private const int EntryLength = 15;

    public static O5 Decode(byte[] buffer)
    {
        ...
        if (buffer.Length < RecordLength)
        {
            throw new ArgumentException(
                $"O5 record requires at least {RecordLength} bytes, got {buffer.Length}.",
                nameof(buffer));
        }
        ...
        if (actualSpec != RecordSpec)
        {
            throw new InvalidOperationException(
                $"Buffer is not an O5 record. Expected RecordSpec '{RecordSpec}', got '{actualSpec}'.");
        }
        ...
        return new O5
        {
            ...
            SanrenpukuFlag = Read(span, 40, 1),

            // Kumi(1,6), Odds(7,6), Ninki(13,3)
            Kumi = ReadStruct(span, 41, EntryCount, EntryLength, 1, 6),
            Odds = ReadStruct(span, 41, EntryCount, EntryLength, 7, 6),
            Ninki = ReadStruct(span, 41, EntryCount, EntryLength, 13, 3),

            TotalHyosuSanrenpuku = Read(span, 12281, 11),
        };
    }

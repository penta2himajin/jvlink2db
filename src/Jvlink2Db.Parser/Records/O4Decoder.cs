...
public static class O4Decoder
{
    public const string RecordSpec = "O4";
    public const int RecordLength = 4031;
    private const int EntryCount = 306;
    private const int EntryLength = 13;

    public static O4 Decode(byte[] buffer)
    {
        ...
        if (buffer.Length < RecordLength)
        {
            throw new ArgumentException(
                $"O4 record requires at least {RecordLength} bytes, got {buffer.Length}.",
                nameof(buffer));
        }
        ...
        if (actualSpec != RecordSpec)
        {
            throw new InvalidOperationException(
                $"Buffer is not an O4 record. Expected RecordSpec '{RecordSpec}', got '{actualSpec}'.");
        }
        ...
        return new O4
        {
            ...
            TotalHyosuUmatan = Read(span, 4019, 11),
        };
    }

...
public sealed class PostgresO5Writer : PostgresBulkWriter<O5>
{
    public PostgresO5Writer(NpgsqlDataSource dataSource, string? schemaName = null)
        : base(dataSource, schemaName, "o5", O5Columns.All, O5Columns.PrimaryKey)
    {
    }

    protected override async Task WriteRowAsync(NpgsqlBinaryImporter writer, O5 r, CancellationToken ct)
    {
        ...
        await WriteText(writer, r.SanrenpukuFlag, ct).ConfigureAwait(false);
        ...
        await WriteLong(writer, r.TotalHyosuSanrenpuku, ct).ConfigureAwait(false);
    }
}

...
public sealed class PostgresO3Writer : PostgresBulkWriter<O3>
{
    public PostgresO3Writer(NpgsqlDataSource dataSource, string? schemaName = null)
        : base(dataSource, schemaName, "o3", O3Columns.All, O3Columns.PrimaryKey)
    {
    }

    protected override async Task WriteRowAsync(NpgsqlBinaryImporter writer, O3 r, CancellationToken ct)
    {
        ...
        await WriteText(writer, r.WideFlag, ct).ConfigureAwait(false);

        await WriteTextArray(writer, r.Kumi, ct).ConfigureAwait(false);
        await WriteIntArray(writer, r.OddsLow, ct).ConfigureAwait(false);
        await WriteIntArray(writer, r.OddsHigh, ct).ConfigureAwait(false);
        await WriteShortArray(writer, r.Ninki, ct).ConfigureAwait(false);

        await WriteLong(writer, r.TotalHyosuWide, ct).ConfigureAwait(false);
    }
}
